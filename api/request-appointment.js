...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!name || phone.length < 10 || !brand || !model || !address || !issue || !/^\d{4}-\d{2}-\d{2}$/.test(preferredDate)) {
      return sendJson(response, 400, { message: "Preencha todos os campos obrigatórios do agendamento." });
    }
    ...
    return sendJson(response, 201, { orderNumber: number });
  } catch (error) {
    return sendJson(response, 500, { message: error.message || "Não foi possível registrar o agendamento." });
  }
}
